...
/**
 * @title AugmentedRedBlackTreeLib
 * @notice Augmented red-black tree supporting composite keys (IR+LTV) and a heap per key.
 *
 * @dev
 * - Zero value (empty) is not supported.
 * - Supports up to 2,147,483,647 values.
 * - IR and LTV are uint64 each, packed into a 256-bit composite key:
 *     * IR:  bits [160..223]
 *     * LTV: bits [ 96..159]
 *   The red-black tree is ordered lexicographically by (IR, then LTV).
 *
 * - For each composite key, multiple entries are stored in a binary heap that:
 *     * Prioritizes larger available liquidity first (max-heap on `amount`).
 *     * Uses earlier timestamp as the tie-breaker (smaller `timestamp` wins).
 *
 * - Portions derived from upstream MIT-licensed code are acknowledged below.
 *
 * @author Avon
 * @notice Based on:
 *   - Solady RedBlackTreeLib (MIT)
 *     https://github.com/vectorized/solady/blob/main/src/utils/RedBlackTreeLib.sol
 *   - BokkyPooBah's RedBlackTreeLibrary (MIT)
 *     https://github.com/bokkypoobah/BokkyPooBahsRedBlackTreeLibrary
 */
library AugmentedRedBlackTreeLib {
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                       CUSTOM ERRORS                        */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    ...
    /// @dev The amount exceeds maximum value (2^128 - 1).
    error AmountExceedsMaximum(uint256 amount);

    /// @dev The timestamp exceeds maximum value (2^64 - 1).
    error TimestampExceedsMaximum(uint256 timestamp);

    /// @dev The account address cannot be zero.
    error InvalidAccount();

    /// @dev Invalid entry index.
    error InvalidEntryIndex(uint256 index);

    /// @dev IR too large.
    error IRTooLarge(uint256 ir);

    /// @dev LTV too large.
    error LTVTooLarge(uint256 ltv);
    ...
    /// @dev A red-black-tree node with augmented data storage
    /// @member nodes Storage slot for tree structure
    /// @member entriesMap Mapping of compositeKey => index => Entry (min-heap)
    struct Tree {
        uint256 _spacer;
        mapping(uint256 => uint256) entryCount;
        mapping(uint256 => mapping(uint256 => Entry)) entriesMap;
        mapping(uint256 => uint256) nodes;
    }

    /// @dev An entry in the heap at a node.
    /// Multiple entries with the same IR+LTV composite key are organized
    /// in a min-heap structure ordered by timestamp.
    struct Entry {
        uint256 amount;
        uint64 timestamp;
        address account;
    ...
    // - [93]      `isRed`
    // - [96..159] `ltv` (64 bits)
    // - [160..223] `interestRate` (64 bits)  // IR moved to more significant bits
    ...
    uint256 private constant _BITPOS_IR = 160;
    uint256 private constant _BITPOS_LTV = 96;
    uint256 private constant _BITMASK_VALUE = 0xFFFFFFFFFFFFFFFF;
    uint256 private constant _BIT_FULL_VALUE_SLOT = 1 << 31;

    // Add constant for bit masks
    uint256 private constant _IR_MASK = 0xFFFFFFFFFFFFFFFF;
    uint256 private constant _LTV_MASK = 0xFFFFFFFFFFFFFFFF;
        ...
        returns (uint256 nodeIndex)
    {
        ...
        uint256 count = tree.entryCount[compositeKey];

        // Add new entry at the end
        tree.entriesMap[compositeKey][count] = Entry(safeAmount, safeTimestamp, account);
        tree.entryCount[compositeKey] = count + 1;

        if (count > 0) {
            // Maintain heap property
            _heapifyUp(tree, compositeKey, count);
        }

        // Insert node if needed
        if (key == 0) {
            uint256 err = _update(nodes, cursor, key, compositeKey, 0);
    ...
    /// For augmented data, returns the composite key.
    function value(bytes32 ptr) internal view returns (uint256 result) {
        if (ptr == bytes32(0)) return result;
        /// @solidity memory-safe-assembly
        assembly {
            let packed := sload(ptr)
            let ir := and(shr(_BITPOS_IR, packed), _BITMASK_VALUE)
            let ltv := and(shr(_BITPOS_LTV, packed), _BITMASK_VALUE)

            result := or(shl(_BITPOS_IR, ir), shl(_BITPOS_LTV, ltv))
        }
    }
    ...
    /// @dev Removes an entry and updates aggregates
    /// @param tree The tree containing the entry
    /// @param compositeKey The composite key (packed IR and LTV values)
    /// @param index The index of the entry in the min-heap (0-based)
    /// @dev Reverts with InvalidEntryIndex if index is out of bounds
    function removeEntry(Tree storage tree, uint256 compositeKey, uint256 index) internal {
        uint256 count = tree.entryCount[compositeKey];

        if (index >= count) revert InvalidEntryIndex(index);
        ...
        if (index != lastIndex) {
            ...
            if (lastIndex > 0) {
                ...
                if (index == 0) {
                    ...
                    Entry memory currentEntry = tree.entriesMap[compositeKey][index];
                    Entry memory parentEntry = tree.entriesMap[compositeKey][parentIndex];

                    if (
                        currentEntry.amount > parentEntry.amount
                            || (currentEntry.amount == parentEntry.amount && currentEntry.timestamp < parentEntry.timestamp)
                    ) {
                        _heapifyUp(tree, compositeKey, index);
                    } else {
                        _heapifyDown(tree, compositeKey, index);
                    }
                }
    ...
    /// @dev Heapify upward from index to maintain min-heap property
    /// The min-heap is now primarily ordered by available liquidity (descending), with larger liquidity at the root.
    /// In case of equal liquidity, timestamp is used as a secondary sorting criterion.
    /// @param tree The tree containing the heap
    /// @param compositeKey The composite key for the entry's IR+LTV
    /// @param index The index to heapify up from
    function _heapifyUp(Tree storage tree, uint256 compositeKey, uint256 index) internal {
        if (index == 0) return;

        Entry memory entry = tree.entriesMap[compositeKey][index];
        uint256 availableLiquidity = entry.amount;
        uint256 timestamp = entry.timestamp;

        uint256 current = index;
        while (current > 0) {
            uint256 parentIndex = (current - 1) >> 1;
            Entry memory parent = tree.entriesMap[compositeKey][parentIndex];

            // Prioritize by available liquidity (descending order - larger values come first)
            // Only use timestamp as a tiebreaker when liquidity amounts are equal
            if (
                parent.amount > availableLiquidity
                    || (parent.amount == availableLiquidity && parent.timestamp <= timestamp)
            ) break;

            // Swap with parent
            tree.entriesMap[compositeKey][current] = parent;
            current = parentIndex;
        }

        if (current != index) {
            tree.entriesMap[compositeKey][current] = entry;
        }
    }

    /// @dev Heapify downward from index to maintain min-heap property
    /// The min-heap is now primarily ordered by available liquidity (descending), with larger liquidity at the root.
    /// In case of equal liquidity, timestamp is used as a secondary sorting criterion.
    /// @param tree The tree containing the heap
    /// @param compositeKey The composite key for the entry's IR+LTV
    /// @param index The index to heapify down from
    /// @dev This function maintains the heap invariant after an entry is removed
    function _heapifyDown(Tree storage tree, uint256 compositeKey, uint256 index) internal {
        uint256 count = tree.entryCount[compositeKey];
        if (count <= 1) return;
        ...
        uint256 largest;
        while (true) {
            uint256 leftChild = (current << 1) + 1;
            if (leftChild >= count) break;

            uint256 rightChild = leftChild + 1;

            // Determine which child has larger available liquidity
            // If equal, use timestamp as a tiebreaker
            if (rightChild < count) {
                Entry memory leftEntry = tree.entriesMap[compositeKey][leftChild];
                Entry memory rightEntry = tree.entriesMap[compositeKey][rightChild];

                if (
                    rightEntry.amount > leftEntry.amount
                        || (rightEntry.amount == leftEntry.amount && rightEntry.timestamp < leftEntry.timestamp)
                ) {
                    largest = rightChild;
                } else {
                    largest = leftChild;
            ...
            Entry memory largestEntry = tree.entriesMap[compositeKey][largest];
            if (
                largestEntry.amount < availableLiquidity
                    || (largestEntry.amount == availableLiquidity && largestEntry.timestamp >= timestamp)
            ) break;

            // Swap with largest child
            tree.entriesMap[compositeKey][current] = tree.entriesMap[compositeKey][largest];
            current = largest;
        }
        ...
        returns (uint256 err)
    {
        ...
        assembly {
            ...
            function insertImpl(nodes_, cursor_, key_, x_) -> err_ {
                if key_ {
...

                {
                    ...
                    let ir := and(shr(160, x_), _IR_MASK)
                    let ltv := and(shr(96, x_), _LTV_MASK)

                    packed_ := or(packed_, shl(_BITPOS_IR, ir))
                    packed_ := or(packed_, shl(_BITPOS_LTV, ltv))
                    ...
                    for {} 1 {} {
                        ...
                        let cIR := and(shr(_BITPOS_IR, cPacked_), _IR_MASK)
                        let cLTV := and(shr(_BITPOS_LTV, cPacked_), _LTV_MASK)
                        let searchIR := and(shr(160, x_), _IR_MASK)
                        let searchLTV := and(shr(96, x_), _LTV_MASK)

                        if eq(searchIR, cIR) {
                            if lt(searchLTV, cLTV) { sstore(s_, setKey(cPacked_, _BITPOS_LEFT, totalNodes_)) }
                            if iszero(lt(searchLTV, cLTV)) { sstore(s_, setKey(cPacked_, _BITPOS_RIGHT, totalNodes_)) }
                        }
                        if iszero(eq(searchIR, cIR)) {
                            if lt(searchIR, cIR) { sstore(s_, setKey(cPacked_, _BITPOS_LEFT, totalNodes_)) }
                            if iszero(lt(searchIR, cIR)) { sstore(s_, setKey(cPacked_, _BITPOS_RIGHT, totalNodes_)) }
                        }
                        break
                    }
                }
            ...
            function remove(nodes_, key_) -> err_ {
                ...
                if iszero(eq(cursor_, key_)) {
                    ...
                    // Copy `left`, `right`, `red` and preserve IR/LTV bits
                    // forgefmt: disable-next-item
                    let mask_ := sub(shl(_BITPOS_PACKED_VALUE, 1), 1)
                    sstore(or(nodes_, cursor_), or(and(cursorPacked_, not(mask_)), and(packed_, mask_)))
                ...
                let lastIR := and(shr(_BITPOS_IR, lastPacked_), _IR_MASK)
                let lastLTV := and(shr(_BITPOS_LTV, lastPacked_), _LTV_MASK)

                let cursorPacked := sload(or(nodes_, cursor_))
                let cursorIR := and(shr(_BITPOS_IR, cursorPacked), _IR_MASK)
                let cursorLTV := and(shr(_BITPOS_LTV, cursorPacked), _LTV_MASK)

                let sameKey := and(eq(lastIR, cursorIR), eq(lastLTV, cursorLTV))
            ...
            for {} 1 {} {
                if iszero(mode) {
                    err := insertImpl(nodes, cursor, key, x)
                    break
                }
    ...
    function _find(Tree storage tree, uint256 x) private view returns (uint256 nodes, uint256 cursor, uint256 key) {
        ...
        assembly {
            ...
            let searchIR := shr(160, x)
            let searchLTV := and(shr(96, x), _BITMASK_VALUE)
            ...
            for { let probe := shr(128, sload(nodes)) } probe {} {
                ...
                let nodeIR := and(shr(_BITPOS_IR, nodePacked), _BITMASK_VALUE)
                let nodeLTV := and(shr(_BITPOS_LTV, nodePacked), _BITMASK_VALUE)

                if and(eq(nodeIR, searchIR), eq(nodeLTV, searchLTV)) {
                    key := cursor
                    break
                }

                let direction := 0
                if or(gt(searchIR, nodeIR), and(eq(searchIR, nodeIR), gt(searchLTV, nodeLTV))) { direction := 1 }

                probe := and(shr(mul(direction, _BITPOS_RIGHT), nodePacked), _BITMASK_KEY)
            }
        }
    }
    ...
    function _packCompositeKey(uint256 ir, uint256 ltv) internal pure returns (uint256) {
        ...
        return (ir << _BITPOS_IR) | (ltv << _BITPOS_LTV);
    }

    /// @dev Helper function to unpack a composite key into IR and LTV
    function _unpackCompositeKey(uint256 compositeKey) internal pure returns (uint256 ir, uint256 ltv) {
        ir = (compositeKey >> _BITPOS_IR) & _IR_MASK;
        ltv = (compositeKey >> _BITPOS_LTV) & _LTV_MASK;
    }

    /// @dev Retrieves a specific entry from the min-heap
    /// @param compositeKey Packed uint256 containing IR (bits 160-223) and LTV (bits 96-159)
    /// @return The entry at the specified index
    function getEntryAt(Tree storage tree, uint256 compositeKey, uint256 index) internal view returns (Entry storage) {
        if (index >= tree.entryCount[compositeKey]) revert InvalidEntryIndex(index);
        return tree.entriesMap[compositeKey][index];
    }

    /// @dev Get the number of entries for a composite key
    function getEntryCount(Tree storage tree, uint256 compositeKey) internal view returns (uint256) {
